...
- Test suite passed 36/36 files and 184/184 tests.
...
- Supabase is active and healthy. RLS and least-privilege grants were verified read-only. Current advisor warnings are documented and must be reviewed again after final Auth configuration.
- The synchronized privacy, terms, support and homepage source is ready in `pawpair-site/`. The live deployment still shows the July 21 copy and must be updated before submission.
...
- [ ] Enable Anonymous Sign-ins in the PawPair Supabase project.
- [ ] Add native-compatible abuse protection, or approve conservative anonymous sign-up rate limits plus monitoring and abandoned-user cleanup.
...
- [ ] Create or verify RevenueCat entitlement `premium` and offering `default`.
- [ ] Attach `app.pawpair.medtracker.premium.annual` and `app.pawpair.medtracker.premium.monthly` to the default offering.
- [ ] Set the production RevenueCat iOS public SDK key in the EAS production environment.
...
- [ ] Deploy `pawpair-site/` and confirm the live Privacy and Terms pages show **Effective July 23, 2026**.
