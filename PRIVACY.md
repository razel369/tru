...
If you view, purchase or restore PawPair Premium, RevenueCat processes an anonymous app user identifier, product and offering identifiers, purchase receipts, subscription status, store, platform and limited device or app metadata needed to validate and manage the subscription. Apple processes payment and App Store account information.

PawPair does not send pet profiles, care routines, health records, medical files, contact details or backup contents to RevenueCat. RevenueCat acts as PawPair's subscription infrastructure provider and retains subscription records as needed to validate purchases, prevent fraud and provide subscription analytics. Apple may retain transaction records under its own legal and accounting obligations.

To request deletion of PawPair-controlled subscription data, contact raz@rmalk.co.il.

## 8. Supabase and other third parties

Supabase provides anonymous authentication and the optional analytics database. It processes the anonymous user identifier, installation identifier and the limited analytics fields described above. RevenueCat and Apple process subscription information as described in section 7.

PawPair does not send care data to advertising partners. Apple may process App Store, device backup, crash or diagnostic information under Apple's own terms and privacy policies when you use Apple services or choose to share diagnostics with Apple.
